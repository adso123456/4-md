...
**为什么是"部分同步"？** 完全异步模型下共识不可能（FLP 定理），完全同步模型又太理想化。部分同步是折中——假设消息最终会到，但不承诺多快到。工程上的体现：

| 同步模型的做法 | Zyzzyva 的部分同步做法 |
|--------------|----------------------|
| 超时 = 故障，立即切换 | 超时先**怀疑**，重试一次（Case 3.1） |
| 精确计时驱动协议推进 | 设一个足够大的超时，超时后还不行才判定拜占庭（Case 3.2） |
...
if 2f + 1 <= |S| <= 3f then
// 有 replica 没响应（可能拜占庭），不够全票，触发慢路径
