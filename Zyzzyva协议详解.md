...
| 多数派交集约束 | 两个 correct 节点的多数派必须相交。为什么交集大小必须 **> f**？因为如果交集 ≤ f，f 个拜占庭节点可以全部落在交集中，一致对外说谎，而交集内没有任何 correct 节点来纠正它们。所以要保证交集 ≥ f+1： |
| 约束推导 | (n-f) + (n-f) − n ≥ f+1 → 2n − 2f − n ≥ f+1 → n ≥ 3f+1 |
| 结论 | **n ≥ 3f+1** |
...
### 2.4 四节点拜占庭共识算法（Algorithm 4.1）

PPT 给出了 n=4, f=1 的具体算法，这是理解拜占庭共识的第一步：

```
Algorithm 4.1  Byzantine agreement with f = 1
Code for node u, with input value x

Round 1
  Send tuple(u, x) to all other nodes
  // 把自己的输入广播出去，让所有节点知道"我看到了什么"

  Receive tuple(v, y) from all other nodes v
  Store all received tuple(v, y) in a set Su
  // 收集别人的输入，为第二轮"互相验证"做准备

Round 2
  Send set Su to all other nodes, including own Su
  // 把自己收集到的集合再广播出去，让节点对比彼此收到的信息

  Receive sets Sv from all nodes v
  T = { tuple(v, y) | seen in at least two sets Sv, including own Su }
  // 取至少在两个集合中出现的值：f=1 时，2 票意味着至少有 1 票来自 correct 节点

  Let tuple(v, y) in T be the tuple with the smallest value y
  Decide on value y
  // 选 y 值最小的 tuple；y 相同时所有 correct 节点选同一个，保证 agreement
```

> **关键**：correct 节点通过 T 集获得 correct-input validity（所有 correct 值都在 T 中）。拜占庭节点如果向不同节点发不同值，这些值无法满足"至少出现在两个集合中"。
...
| 维度 | 设定 |
|------|------|
| 网络模型 | 部分同步：消息最终会到达，但延迟上限未知（不像同步模型有精确上限 Δ） |
| 通信模式 | 两种：等待多数派回复（如客户端收 2f+1 个响应）、向全体广播（如 primary 发 ORDER-REQ） |
| 认证 | 节点可认证身份（数字签名），拜占庭节点**不能**破解签名 |

**为什么是"部分同步"？** 异步下共识不可能（FLP），同步太理想化。部分同步是工程折中——消息最终会到，但不承诺多快到。体现为：超时先怀疑再重试（Case 3.1），不行才判拜占庭（Case 3.2），而非一超时就切主。
...
### 5.1 PPT 符号定义（Message Notations）

| 符号 | 定义 |
|------|------|
| `d` | H(m)，消息 m 的哈希值 |
| `<m>_σi` | 消息 m 附带节点 i 的签名：`<m, σi = sig_ski(m)>` |
| `ci` | Operation / Command，操作或命令 |
| `t` | timestamp，时间戳 |
| `cid` | client id，客户端标识 |
| `i` | replica id，副本标识 |
| `v` | View number，逻辑时钟 |
| `sn` | Sequence number，单调递增的序列号 |
| `hn` | 序列号到 sn 为止的命令历史摘要，`hn = H(hn-1, d)`，`h0 = ε`（空值） |
...
| 消息 | 格式 | 说明 |
|------|------|------|
| REQUEST | `<REQUEST, ci, t, cid>_σclient` | 客户端向 primary 发起请求 |
| ORDER-REQ | `<ORDER-REQ, v, sn, hn, ci, t, cid, ND>_σprimary` | primary 向所有 replica 广播排序请求 |
| SPEC-RESP | `<SPEC-RESP, v, sn, hn, i, cid, result>_σreplica` | replica 执行后向客户端返回结果 |
| COMMIT | `<COMMIT, v, sn, result>_σreplica` | 客户端向 replica 发送提交确认 |
| LOCAL-COMMIT | `<LOCAL-COMMIT, hn, i, cid>_σreplica` | replica 向客户端确认本地已提交 |

> **注意**：签名保证消息来源可验证且不可伪造，这是 BFT 区别于 CFT（Paxos/Raft）的关键。另外 `ND` 是 non-deterministic 变量，用于处理不确定性操作，仅在 ORDER-REQ 中出现。

**协议描述中使用的简写对照**（PPT 自身在流程图里用了简写，后续章节沿用）：
...
以下伪代码来自 PPT 原版，涵盖一次完整请求从发起到 Case 1 完成的全过程：

```
Zyzzyva: basic protocol

At time t client u wants to execute command c
Client u sends a request R = Request(c, t)u to primary p
// 客户端发起：向 primary 发送带时间戳的请求

Primary p appends c to its local history, i.e., hp = (hp, c)
Primary p sends OR = OrderedRequest(hp, c, R)p to all replicas
// primary 排序：将命令追加到自己的历史，广播排序请求

Each replica r appends command c to local history hr = (hr, c)
    and checks whether hr = hp
Each replica r runs command c and obtains result a
Each replica r sends Response(a, OR)r to client u
// replica 执行：追加历史、检查一致性、执行命令、返回结果

Client u collects the set S of received Response(a, OR)r messages
Client u checks if all histories hr are consistent
// 客户端收集：收集所有 replica 的响应，检查历史是否一致

-- continue: no failures (Case 1) --

if |S| = 3f + 1 then       // consistent
    client u considers command c to be complete
end if
// 判定：3f+1 全票一致 → 命令完成，零共识开销
```

**为什么 3f+1 一致就安全？** n=3f+1，3f+1 个一致意味着所有 correct replica（2f+1 个）都收到了相同的 order，即使 primary 想推翻也来不及了。

### 6.2 Case 2：慢路径（响应不足 3f+1）

**条件**：客户端收到 2f+1 ~ 3f 个一致响应，不够 3f+1。注意这不等于"有拜占庭节点"——可能只是网络延迟。Case 1 中拜占庭 replica 也可能正常响应蒙混过关。

```
-- continue: Byzantine Replicas (Case 2) --

if 2f + 1 <= |S| <= 3f then
// 响应不足全票，可能因为网络延迟或个别 replica 不响应，触发慢路径
    Client u sends Commit(S)u to all replicas
// 客户端把 2f+1 个签名打包成 Commit Certificate (CC) 发出去
    each replica r replies with a LocalCommit(S)r message to u
// replica 验证 CC 与本地历史一致后确认提交
    client u collects at least 2f + 1 LocalCommit(S)r messages
        and considers c to be complete
// 2f+1 个 LOCAL-COMMIT 证明至少 f+1 个 correct 节点已确认 → 完成
end if
```
...
```
-- Zyzzyva: Faulty Primary --

if |S| < 2f + 1 then
// 响应严重不足，primary 可能故障或作恶
    Client u sends the original R = Request(c, t)u to all replicas
// 绕开 primary，直接向所有 replica 重发请求
    each replica r sends a ConfirmRequest(R)r message to p
// replica 向 primary 确认"我收到了这个请求"
    if primary p replies with OR then                     // Case 3.1
// primary 只是慢，补发 ORDER-REQ 后恢复正常
        replica r forwards OR to all replicas
        each replica r appends command c to local history hr = (hr, c)
            and checks whether hr = hp
        each replica r runs command c and obtains result a,
            sends Response(a, OR)r to client u
        client u collects the set S of received Response(a, OR)r messages
        client u checks if all histories hr are consistent
// 流程回到 Case 1 或 Case 2
    else                                                  // Case 3.2
// primary 不响应 → 发起 View Change
        replica r initiates view change by broadcasting
            IHatePrimaryr to all replicas
    end if
end if
```
...
**概述**：当前 View v，需切换到 View v+1，新 primary = replica[(v+1) mod n]。

**Step 1 — View Change Agreement**

```
All replicas continuously collect the set H of IHatePrimaryr messages
if a replica r received |H| > f message or a valid ViewChange message then
    // |H| > f: 至少 1 个 I-HATE-PRIMARY 来自 correct 节点；或收到有效 ViewChange 则跟从
    replica r broadcasts ViewChange(Hr, hr, CClr)r
    // ViewChange 包含：已收集的 I-HATE-PRIMARY 集合、本地历史、最新的 CC
    replica r stops participating in the current view
    // 停止参与当前 view，防止拜占庭 primary 继续操纵
    replica r switches to the next primary "p = p + 1"
    // 轮换 primary，进入 view v+1
end if
```

**Step 2 — View Change Execution（新 primary）**

```
The new primary p collects the set C of ViewChange(Hn, hn, CCln)n messages
if new primary p collected |C| >= 2f + 1 messages then
    // >= 2f+1 个 ViewChange → 其中至少 f+1 个来自 correct replica，足够重建历史
    new primary p sends NewView(C)p to all replicas
    // NewView 携带收集到的所有 ViewChange，作为新 primary 合法的证据
end if
```

**Step 3 — 各 Replica 确认新视图**

```
if a replica r received a NewView(C)p message then
    replica r recovers new history hₙₑw as shown in Algorithm history recover
    // 用 History Recovery 算法从 C 中重建统一历史（见第九章）
    replica r broadcasts ViewConfirm(hₙₑw)n message to all replicas
    // 广播对 hₙₑw 的确认，告知其他 replica "我认可这个历史"
end if

if a replica r received 2f + 1 ViewConfirm(hₙₑw)n messages then
    // 2f+1 个确认 → 至少 f+1 个 correct replica 都认可同一历史
    replica r accepts hn = hₙₑw as the history of the new view
    replica r starts participating in the new view
    // 新视图正式生效，恢复处理客户端请求
end if
```

**三步阈值总结**：发起 `|H| > f` 或 1 个有效 ViewChange → 新 primary 收集 `>= 2f+1` 个 ViewChange 后发 NewView → 各 replica 收 `2f+1` 个 ViewConfirm 后进入新视图。
...
```
Zyzzyva: History Recovery

C     = { 2f+1 个 VIEW-CHANGE(Hʳ, hʳ, CCₗʳ)_r 消息 }
    // 来自 NEW-VIEW 消息，包含所有正确节点的本地历史
R     = { C 中包含的 replica }
    // 初始信任集：后续通过 f+1 投票逐次过滤
CCₗ   = C 中最新的 commit certificate
    // Lemma 3: 最新 CC 一定在 C 中（至少 f+1 个 correct 持有）
hₙₑw  = CCₗ 中包含的历史 hₗ
    // 锚点：CCₗ 之前的命令确认已完成，不容丢失
k     = l + 1
    // CCₗ 之后的命令处于灰色地带，需要逐一检查
while C 中存在命令 cₖ do
    if cₖ 被 R 中至少 f+1 个 replica 报告 then
        // Lemma 4: ≥ f+1 报告中至少 1 个来自 correct，证明该命令确实被执行过
        从 R 中移除不支持 cₖ 的 replica
        // 不支持 cₖ 的 replica 可能是拜占庭，收紧信任集，不再采信
        hₙₑw = (hₙₑw, cₖ)
        // 命令被确认安全，追加到重建历史中
    end if
    k = k + 1
    // 关键：不满足条件也不 break，继续检查后续命令（遇挫不停止）
end while
return hₙₑw
```
...
此时 cmd_k 的持有者 = {R0（拜占庭）, R1（correct）} = f+1 = 2 个节点。客户端尚未收到 2f+1 个响应，cmd_k 处于 Gap。

| 步骤 | 事件 |
|------|------|
| 1 | primary R0 向 R1 发送了 cmd_k 的 ORDER-REQ，但故意不发给 R2、R3 |
| 2 | R1 执行了 cmd_k，返回 SPEC-RESPONSE 给客户端 |
| 3 | 客户端只收到 1 个响应（< 2f+1），无法完成；此时 primary R0 崩溃，触发 View Change |
| 4 | **关键**：拜占庭 R0 在它的 VIEW-CHANGE 消息中**声称从未见过 cmd_k** |
| 5 | History Recovery 统计 C 中报告 cmd_k 的 replica：仅 R1 一人 → 1 < f+1=2 |
| 6 | cmd_k 不被纳入 hₙₑw，**命令丢失** |
| 7 | 客户端以为 cmd_k 未执行，重发请求 → 可能被**重复执行**（若 R1 重启后忘了） |

**错误的根因**：History Recovery 依赖"≥ f+1 个 replica 报告命令"来判定其曾被提交过（Lemma 4）。但 Gap 中的命令恰好只有 f+1 个持有者，且其中 f 个是拜占庭——它们串通否认后，只剩 1 个 correct 的孤证，不满足 f+1 阈值。客户端的视角则是混乱的：它不知道命令到底执行了没有。
